...
/**
 * Created by JiangZhuyang on 2017/1/5.
 */

public class GAClient {

    public static void init() {
        ThreadPoolUtils.executeByCached(new ThreadPoolUtils.SimpleTask<AdInfo>() {
            @Override
            public AdInfo doInBackground() throws Throwable {
                try {
                    GAClient.AdInfo adInfo = GAClient.getAdvertisingIdInfo(ToolKit.getApp());
                    if (adInfo != null) {
                        return adInfo;
                    }
                } catch (Exception e) {
            ...
            @Override
            public void onSuccess(AdInfo result) {
                if (result != null && !TextUtils.isEmpty(result.getId())) {
                    SPUtils.getInstance(PhoneUtils.DEVICE_CACHE_NAME).getString("gaid", result.advertisingId);
                } else {
                    SPUtils.getInstance(PhoneUtils.DEVICE_CACHE_NAME).getString("gaid", "null");
                }
            }
        });
    }
