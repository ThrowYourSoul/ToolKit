...
public final class ProcessUtils {
    ...
    public static String getForegroundProcessName() {
        ActivityManager am =
                (ActivityManager) ToolKit.getApp().getSystemService(Context.ACTIVITY_SERVICE);
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            ...
            try {// Access to usage information.
                ApplicationInfo info =
                        pm.getApplicationInfo(ToolKit.getApp().getPackageName(), 0);
                AppOpsManager aom =
                        (AppOpsManager) ToolKit.getApp().getSystemService(Context.APP_OPS_SERVICE);
                if (aom != null) {
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS,
                            info.uid,
                            info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        ToolKit.getApp().startActivity(intent);
                    }
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS,
                            info.uid,
                            info.packageName) != AppOpsManager.MODE_ALLOWED) {
                ...
                UsageStatsManager usageStatsManager = (UsageStatsManager) ToolKit.getApp()
    ...
    public static boolean isMainProcess() {
        return ToolKit.getApp().getPackageName().equals(getCurrentProcessName());
    }
